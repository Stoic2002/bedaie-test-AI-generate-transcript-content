...
import { Trash2, Search, ChevronDown, Clock, Tag, MessageSquare, Sparkles, Undo2, Monitor, Mic, Type, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function HistoryPage() {
  ...
  const handleDelete = async (id: string, e: React.MouseEvent) => {
    ...
    try {
      ...
      toast.success(`${t.deleted} "${itemToDelete.topic || "Item"}"`);
    } catch (err) {
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-32 px-4 text-center">
          ...
          <h3 className="text-[15px] font-medium text-[var(--color-text-primary)] mb-1">
            {debouncedSearch ? t.noResults : t.noHistory}
          </h3>
          <p className="text-[13px] text-[var(--color-text-secondary)]">
            {debouncedSearch ? `${t.noResultsSubtitle} "${debouncedSearch}"` : t.noHistorySubtitle}
          </p>
        </div>
      ) : (
        <>
          <div className="bg-white border border-[var(--color-border)] rounded-xl overflow-hidden shadow-sm shadow-black/5 mb-6">
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between border-t border-[var(--color-border)] pt-6">
              <div className="text-[13px] text-[var(--color-text-secondary)]">
                {t.page} <span className="font-medium text-[var(--color-text-primary)]">{page}</span> {t.of} <span className="font-medium text-[var(--color-text-primary)]">{totalPages}</span>
                <span className="ml-2 opacity-50">({totalItems} items)</span>
              </div>
              <div className="flex items-center gap-2">
                <button
                  onClick={() => setPage((p) => Math.max(1, p - 1))}
                  disabled={page === 1 || loading}
                  className="flex items-center justify-center p-2 rounded-lg border border-[var(--color-border)] hover:bg-slate-50 disabled:opacity-30 disabled:hover:bg-transparent transition-colors"
                >
                  <ChevronLeft size={18} />
                </button>
                <button
